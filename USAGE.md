# Bitácora — usage conventions

Bitácora is a context-capture and recall layer, not a system of record. The conventions below cover the three places where that distinction matters most for teams whose work passes through formal review, change control, or audit.

## 1. `/improve` on formally controlled requirement tickets

The `/improve` rewrite **replaces** the ticket description in place. Be deliberate: once the rewrite lands, the new description is what downstream review will read.
...
- For tickets that go through formal review (BA-approved requirements, regulated scope, customer commitments), treat `/improve` like an edit by hand: prefer the `--title`-only scope unless you're authoritatively changing the requirement, and share the rewrite for review before accepting at the gate.
...
- For any load-bearing calculation or data point a downstream decision rests on, read the underlying ticket, the `[CTX]` comment trail, and the code or data they cite.
...
In a review-heavy environment this is the line reviewers will care about and the line your future-self will care about. Write it like you're explaining the choice to someone in eight months who hasn't seen the conversation. Cite the constraint (comment number, Slack thread, policy reference) when one exists.
